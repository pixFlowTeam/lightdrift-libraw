...
async function testAllFormats() {
    ...
    try {
            ...
            for (const filename of files) {
                ...
                try {
                    ...
                    if (actualMfg && !actualMfg.includes(expectedMfg)) {
                        console.log(`   ⚠️  Manufacturer mismatch: expected ${expectedMfg}, got ${metadata.make}`);
                    }
                    
                    // Display key information
                    console.log(`   📷 Camera: ${metadata.make} ${metadata.model}`);
                    console.log(`   📐 Resolution: ${size.width} x ${size.height} pixels (${(size.width * size.height / 1000000).toFixed(1)} MP)`);
                    
                    // Camera settings
                    const iso = metadata.iso || 'N/A';
                    const aperture = metadata.aperture ? `f/${metadata.aperture.toFixed(1)}` : 'N/A';
                    const shutter = metadata.shutterSpeed ? `1/${Math.round(1/metadata.shutterSpeed)}s` : 'N/A';
                    const focal = metadata.focalLength ? `${metadata.focalLength}mm` : 'N/A';
                    console.log(`   🎯 Settings: ISO ${iso}, ${aperture}, ${shutter}, ${focal}`);
        ...
        console.log('═'.repeat(60));
        console.log(`📊 Overall: ${totalSuccess}/${totalProcessed} files processed successfully (${((totalSuccess/totalProcessed)*100).toFixed(1)}%)\n`);
        ...
        for (const [groupName, result] of Object.entries(results)) {
            ...
            if (result.samples.length > 0) {
                ...
                const resolutions = [...new Set(result.samples.map(s => s.resolution))];
        ...
        console.log(`   • Tested ${Object.keys(formatGroups).length} different RAW formats`);
        console.log(`   • Multiple camera manufacturers verified`);
        console.log(`   • Resolution range from compact to full-frame sensors`);
        console.log(`   • File sizes from ${Math.min(...Object.values(results).flatMap(r => r.samples.map(s => parseFloat(s.fileSize))))}MB to ${Math.max(...Object.values(results).flatMap(r => r.samples.map(s => parseFloat(s.fileSize))))}MB`);
        
        // Recommendations
        if (totalSuccess === totalProcessed) {
            console.log('\n✅ ALL TESTS PASSED - Ready for production use!');
        } else {
            console.log(`\n⚠️  ${totalProcessed - totalSuccess} files failed processing - Review error details above`);
        }
