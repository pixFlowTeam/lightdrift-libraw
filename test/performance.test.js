...
async function performanceBenchmark() {
    ...
    try {
        // Get all RAW files
        const rawFiles = fs.readdirSync(sampleDir)
            .filter(file => {
        ...
        for (let i = 0; i < rawFiles.length; i++) {
            ...
            console.log(`📊 ${i + 1}/${rawFiles.length}: ${filename} (${fileSizeMB.toFixed(2)} MB)`);
            ...
            try {
                ...
                const closeTime = Number(process.hrtime.bigint() - closeStart) / 1000000;
                
                const totalOperationTime = Number(process.hrtime.bigint() - startTime) / 1000000;
                ...
                console.log(`   ⏱️  Load: ${loadTime.toFixed(1)}ms | Meta: ${metaTime.toFixed(1)}ms | Size: ${sizeTime.toFixed(1)}ms | Close: ${closeTime.toFixed(1)}ms`);
                console.log(`   🚀 Total: ${totalOperationTime.toFixed(1)}ms | Throughput: ${throughputMBps.toFixed(1)} MB/s | ${(pixelThroughput/1000000).toFixed(1)} MP/s`);
                console.log('   ✅ Success\n');
        ...
        console.log(`📊 Average per File: ${(totalTime/successfulResults.length).toFixed(1)}ms`);
        
        // Timing breakdown
        const avgLoad = successfulResults.reduce((sum, r) => sum + r.loadTime, 0) / successfulResults.length;
        const avgMeta = successfulResults.reduce((sum, r) => sum + r.metaTime, 0) / successfulResults.length;
        const avgSize = successfulResults.reduce((sum, r) => sum + r.sizeTime, 0) / successfulResults.length;
        const avgClose = successfulResults.reduce((sum, r) => sum + r.closeTime, 0) / successfulResults.length;
        ...
        const avgThroughput = successfulResults.reduce((sum, r) => sum + r.throughputMBps, 0) / successfulResults.length;
        const maxThroughput = Math.max(...successfulResults.map(r => r.throughputMBps));
        const minThroughput = Math.min(...successfulResults.map(r => r.throughputMBps));
        ...
        Object.entries(formatStats).forEach(([format, stats]) => {
            ...
            console.log(`   • ${format}: ${avgTime.toFixed(1)}ms avg (${avgThroughput.toFixed(1)} MB/s)`);
        });
        ...
        const resolutionGroups = {
            ...
            'Medium (16-24MP)': successfulResults.filter(r => r.megapixels >= 16 && r.megapixels < 24),
            'High (≥ 24MP)': successfulResults.filter(r => r.megapixels >= 24)
        };
        
        Object.entries(resolutionGroups).forEach(([group, files]) => {
            if (files.length > 0) {
                const avgTime = files.reduce((sum, f) => sum + f.totalTime, 0) / files.length;
                const avgPixelThroughput = files.reduce((sum, f) => sum + f.pixelThroughput, 0) / files.length;
                console.log(`   • ${group}: ${avgTime.toFixed(1)}ms avg (${avgPixelThroughput.toFixed(1)} MP/s)`);
            }
        });
        ...
        if (avgThroughput > 50) {
            console.log('   • ✅ Excellent throughput - suitable for batch processing');
        } else if (avgThroughput > 20) {
            console.log('   • ✅ Good throughput - suitable for real-time applications');
        } else {
